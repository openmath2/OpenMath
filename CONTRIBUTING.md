...
`pnpm install` 시 [husky](https://typicode.github.io/husky/)가 자동으로 git hooks를 설치한다. 이후 commit/push 시 다음이 자동 실행된다:

| 시점 | 도구 | 검사 |
|------|------|------|
| `pre-commit` | lint-staged | 변경된 TS 파일 typecheck, 변경된 Python 파일 ruff lint |
| `pre-push` | husky | main 브랜치 직접 push 차단, Node/Python 전체 테스트 |
| `commit-msg` | husky | 커밋 메시지 첫 줄 72자 이하 |
